...
void graph_traversal(int t, int v, vector <int> visited_v) {
    if ((v == 0) and (t <= 220)) {                              //220 так как если мы опоздаем на 40 минут,
    ...
    if ((t >= 220) or (visited_v.size() > 22)) return;     //22 максимум вершин мы сможем посетить, (0-(12)-0-(12)-0-(12)-..-0
    else {
        for (auto с: g[v]) {
            int updated_time = t + times[v][с];
            vector <int> updated_visited;
            updated_visited.push_back(с);
            for (auto x: visited_v) {
                updated_visited.push_back(x);
            }
            graph_traversal(updated_time, с, updated_visited);
